...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allowedlegalpositions = self.legalPositions
        ...
        if not noisyDistance is None:
            for legpos in allowedlegalpositions:
                allPossible[legpos] = ((emissionModel[util.manhattanDistance(legpos, pacmanPosition)]) * (self.beliefs[legpos]))
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        BafterTimeUpdate = util.Counter()
        allowedlegalpositions = self.legalPositions

        for op in allowedlegalpositions:
            
            NPD  =  self.getPositionDistribution(self.setGhostPosition(gameState, op))

            
            for nposprob in NPD.items():
                BafterTimeUpdate[nposprob[0]]  =BafterTimeUpdate[nposprob[0]]+(nposprob[1]*(self.beliefs[op]))
        


        self.beliefs = BafterTimeUpdate
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numofparticle =  self.numParticles
        tracker = 0
        #print "legalpositions" ,legalpositions
        #print "legalpositions len - 1", legalpositions[(len(legalpositions) - 1)]
        for p in range(numofparticle):
            if tracker < len(legalpositions):
                self.partlist.append(legalpositions[tracker])
                tracker = tracker+1
            else:
                tracker = 0
                self.partlist.append(legalpositions[tracker])
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        parweig= util.Counter()
        "*** YOUR CODE HERE ***"
        if  noisyDistance is None:
             for i in range(self.numParticles):
                self.partlist[i]  = self.getJailPosition()
            
        else:
            for p in self.partlist:
                parweig[p] = parweig[p]+(emissionModel[util.manhattanDistance(pacmanPosition, p)]/self.numParticles)
            
            if parweig.totalCount() == 0:
                self.initializeUniformly(gameState)

            else:
                for i in range(self.numParticles):
                    self.partlist[i] = util.sample(parweig)
    ...
    def elapseTime(self, gameState):
        ...
        for p in range(self.numParticles):
            pafterelapsedtime.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.partlist[p]))))
        self.partlist  =pafterelapsedtime
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        self.ppos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(self.ppos)
        no_ppos = len(self.ppos)
        tracker = 0
        for n in range(self.numParticles):
            if tracker<no_ppos:
                self.particles.append(self.ppos[tracker])
                tracker = tracker+1
            else:
                tracker = 0
                self.particles.append(self.ppos[tracker])
        ...
        for p in self.particles:
            for g in range(self.numGhosts):
                if noisyDistances[g] is None:
                    ...
                    em = emissionModels[g]
                    wgt = ((wgt) * (em[util.manhattanDistance(p[g], pacmanPosition)]))
            partBelief[p] = partBelief[p] + wgt
            self.beliefs = partBelief
            wgt = 1
        if partBelief.totalCount() == 0:
            self.initializeParticles()
            for g in range(self.numGhosts):
                if noisyDistances[g] is None:
                    for n in range(self.numParticles):
                        self.particles[n] = self.getParticleWithGhostInJail(self.particles[n],g)
        else:
            self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for g in range(self.numGhosts):
                d = getPositionDistributionForGhost(
             setGhostPositions(gameState, newParticle), g, self.ghostAgents[g]
          )
                newParticle[g]=  util.sample(d)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            Disbel[particle]   = (Disbel[particle]+ (1.0 / self.numParticles))
        return Disbel
