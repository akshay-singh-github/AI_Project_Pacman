...
def enhancedFeatureExtractorDigit(datum):
    ...
    firstBrightPixel = 0
    LastBrightPixel = 0
    brightPixelinUpperHalf = 0
    firstTopBrightPixel = 0
    firstBottomBrightPixel = 0
    BrightpixelsinleftHalf = 0
    i = 0
    while i < len(pixels):
        pixelrow = pixels[i]
        j = 1
        ...
        while k>0:
            if pixelrow[k] != 0:
                if (LastBrightPixel == 0) or (k > LastBrightPixel):
                    LastBrightPixel = k
            k -= 1
        while j < len(pixelrow):
            if pixelrow[j] != 0:
                noOfBrightPixels += 1
                if j <= (len(pixels) + 1) / 2:
                    brightPixelinUpperHalf += 1
                if (firstBrightPixel == 0) or (j < firstBrightPixel):
                    firstBrightPixel = j
            if pixelrow[j] != pixelrow[j - 1]:
                picturedis += 1
            j += 1
        i += 1
    ...
    while j < len(pixels[0]):
        i = 1
        c = [h[j] for h in pixels]
        g = len(c) - 1
        while g > 0:
            if c[g] != 0:
                if (firstBottomBrightPixel == 0) or (g > firstBottomBrightPixel):
                    firstBottomBrightPixel = g          
            g -= 1
        while i < len(c):
            if c[j] != 0:
                noOfBrightPixels += 1
                if i <= (len(pixels[0]) + 1) / 2:
                    BrightpixelsinleftHalf += 1
                if (firstTopBrightPixel == 0) or (i < firstTopBrightPixel):
                    firstTopBrightPixel = i           
            if c[i] != c[i - 1]:
                picturedis += 1
            i += 1
    ...
    height = firstBottomBrightPixel - firstTopBrightPixel
    width = (LastBrightPixel - firstBrightPixel)    
    AR = (width) / float (height)
    pixelsinupperhalf = (brightPixelinUpperHalf) / float(noOfBrightPixels)
    pixelsinlefthalf = (BrightpixelsinleftHalf) / float(noOfBrightPixels)


    
    for s in range(5):
        if pixelsinlefthalf < 0.28:
            features['pixelsinlefthalf-{i}'.format(i=s)] = 1.0
        else:
            features['pixelsinlefthalf-{i}'.format(i=s)] = 0.0
        
    
    for s in range(10):
        if pixelsinupperhalf > 0.40:
            features['pixelsinupperhalf-{i}'.format(i=s)] = 1.0
        else:
            features['pixelsinupperhalf-{i}'.format(i=s)] = 0.0
            
    
    for s in range(10):
        if AR < 0.845:
            features['AR-{i}'.format(i=s)] = 1.0
        else:
            features['AR-{i}'.format(i=s)] = 0.0
            
        
    for s in range(5):
        if picturedis > 180 :
            features['picturedis-{i}'.format(i=s)] = 1.0
        else:
            features['picturedis-{i}'.format(i=s)] = 0.0
            
        
    for s in range(5):
        if noOfBrightPixels > 295:
            features['noOfBrightPixels-{i}'.format(i=s)] = 1.0
        else:
            features['noOfBrightPixels-{i}'.format(i=s)] = 0.0
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    positionOfGhosts = successor.getGhostPositions()
    positionOfcapsules = successor.getCapsules()
    TotalCapsuleCount = len(successor.getCapsules())
    positionOfPacman = successor.getPacmanPosition()
    TotalFood = successor.getFood()
    positionOfFood = []
    TotalFoodCount = successor.getNumFood()
    NoOfGhosts = len(successor.getGhostPositions())
    DistancefromAllghosts = []
    DistancefromAllCapsules = []
    DistancefromAllFood = []
    minGhostDistance = 9999999999999999999999999
    minCapsuleDistance = 9999999999999999999999999
    
    

    for pog in positionOfGhosts:
        minGhostDistance = min(util.manhattanDistance(positionOfPacman, pog),minGhostDistance)

    for poc in positionOfcapsules:
        minCapsuleDistance=min(util.manhattanDistance(positionOfPacman, poc),minCapsuleDistance)

    for x in range(TotalFood.width):
        for y in range(TotalFood.height):
            if not TotalFood[x][y] is False:
                positionOfFood.append((x,y))

    for pof in positionOfFood:
        DistancefromAllFood.append(util.manhattanDistance(positionOfPacman, pof))
        
                
    
    for k in range(len(DistancefromAllFood)):
        features['DistanceFromThe-{i}thFood'.format(i=k)] = 1/(float)(1+DistancefromAllFood[k])
    
    
    features["minGhostDistance"] = 1/(float) (1+minGhostDistance)
    features["minCapsuleDistance"] = 1/(float)(1+minCapsuleDistance)
    features["TotalFoodCount"] = TotalFoodCount
    features["TotalCapsuleCount"] = TotalCapsuleCount
